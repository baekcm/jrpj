...
function initNavigation2(seq,subq) {
	...
	if(seq==1) {
		...
		switch(subq) {
			case 1:
				tmpalt = "소중한만남 건강한 서비스 올림픽기념국민 생활관";
			break;
			case 2:
				tmpalt = "행복이 숨쉬는 스포츠 문화공간! 종로구민회관";
			break;
			case 3:
				tmpalt = "고객과 함께해서 행복한 종로문화체육센터";
			break;
			case 4:
				tmpalt = "행복하고 건강한 문화스포츠의 여성체육공간! 동부여성문화센터";
			break;
			case 5:
				tmpalt = "독서와 건강을 함께 생각하는 청소년문화센터";
			break;
			case 6:
				tmpalt = "사계절 자연경관을 몸으로 체험하는 삼청테니스장";
			break;
		}
		if(tmpalt) $('#visual > img').attr('alt',tmpalt);
	}
	if(seq>1) {
		$('#visual > img').attr('src',$('#visual > img').attr('src').replace('01','0'+seq));
		switch(seq) {
			case 2:
				tmpalt = "안정적인 주차공간과 편익을 제공하기 위해 노력하겠습니다.";
			break;
			case 3:
				tmpalt = "구민과 함께 소통하고 사랑받는 시설이 되도록 노력하겠습니다.";
			break;
			case 4:
				tmpalt = "고객을 가족처럼 웃음으로 하는 혁신 고객만족1등 공기업 종로구시설관리공단";
			break;
			case 5:
				tmpalt = "구민들과 함께하는 열린마당 언제나 열린마음으로 함께하겠습니다.";
			break;
			case 6:
				tmpalt = "종로구시민들에게 경영의 효율성을 향상시키겠습니다.";
			break;
			case 7:
				tmpalt = "눈과 귀를 열고 고객만족 1등 공기업을 만들기 위해 최선을 다하겠습니다.";
			break;
		}
		if(tmpalt) $('#visual > img').attr('alt',tmpalt);
	}
...
function docTitleNavi(n1,n2,n3,n4) {
	...
	else if(pName.indexOf('_memtype.jsp')>0) { pSort = "회원유형선택"}
	else if(pName.indexOf('_agree')>0) { pSort = "약관동의"}
	else if(pName.indexOf('_certy')>0) { pSort = "회원인증"}
	else if(pName.indexOf('_input')>0) { pSort = "회원정보입력"}
	else if(pName.indexOf('_complete')>0) { pSort = "가입완료"}
	else { pSort =""}
	
	
	if(n1==1){
		//종로시설관리공단 문화체육시설만 4depth인관계로 별도 관리
		if (n4 == 0 ){
			//4depth가 0일때의 처리
			if(n3==0){
				navi = "<a href=\"/index.jsp\"><img src=\"/resources/include/commonfile/image/common/icon_home.gif\" alt=\"홈\" /></a> &gt; "+ $('#gnb'+n1+ '>a > img').attr('alt') + " &gt;  " + $('#gnb'+n1+'_'+n2+' > a').text() + "</strong>";
			...
			navi = "<a href=\"/index.jsp\"><img src=\"/resources/include/commonfile/image/common/icon_home.gif\" alt=\"홈\" /></a> &gt; "+ $('#gnb'+n1+ '>a > img').attr('alt') + " &gt; " + $('#gnb'+n1+'_'+n2+' > a').text() + " &gt;  " + $('#gnb'+n1+'_'+n2+'_'+n3+' > a').text() + "</strong>";
			...
			navi = "<a href=\/index.jsp\"><img src=\"/resources/include/commonfile/image/common/icon_home.gif\" alt=\"홈\" /></a> &gt; "+ $('#gnb'+n1+ '>a > img').attr('alt') + " &gt; " + $('#gnb'+n1+'_'+n2+' > a').text() + " &gt; " + $('#gnb'+n1+'_'+n2+'_'+n3+' > a').text() + " &gt; " + $('#gnb'+n1+'_'+n2+'_'+n3+' li:eq('+n4+') >a ').text() + "</strong>";
		...
		if (n3 == 0 ){
			...
			navi = "<a href=\/index.jsp\"><img src=\"/resources/include/commonfile/image/common/icon_home.gif\" alt=\"홈\" /></a> &gt; "+ $('#gnb'+n1+ '>a > img').attr('alt') + " &gt; " + $('#gnb'+n1+'_'+n2+' > a').text() + " &gt; " + $('#gnb'+n1+'_'+n2+' > ul > li:eq('+n4+')').text(); + "</strong>";
			subDocTitle = $('#gnb'+n1+'_'+n2+ ' >a ').text();
			docTitle = subDocTitle + " "+ pSort +"(" + $('#gnb'+n1+'_'+n2+' > ul > li:eq('+n3+')').text() +")";
			pageTitle = $('#gnb'+n1+'_'+n2+' > ul > li:eq('+n3+')').text();
		}
	}
...
function makeLnb(n1,n2,n3) {
	var userArea = 12;  //GNB에 있는 메뉴인지 여부를 판단하기 위해 부여하는 숫자. 
	if (n1< userArea){
		if(n1==1){ //종로구 시설관리공단 사이트의 특정 메뉴 영역 처리
			var h2Text = $('#gnb'+n1+'>a > img').attr('alt');
			h2Text = "문화체육. "
			var h2Text2 = $('#gnb'+n1+'_'+n2+'>a').text();
			var h2html = "<img src='/resources/include/commonfile/image/title/h2title"+n1+"_"+n2+".gif' alt='"+h2Text+h2Text2+"'>";
			...
			$('#lnb > li').each(function(index, element) {
				if(!$(this).attr('id')) {
					...
					var tmpR = "lnb1_"+n2+"_";;
					var tmpNo= $(this).attr('id').replace(tmpR,'');
					$(this).attr('id','lnb' + tmpNo + '_0');
				}
			...
			var h2Text = $('#gnb'+n1+'>a > img').attr('alt');
			var h2html = "<img src='/resources/include/commonfile/image/title/h2title"+n1+".gif' alt='"+h2Text+"'>";
			$('#contleft > h2').html(h2html);
