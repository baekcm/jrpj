...
public class cert_update_op extends XecureHttpServlet 
{ 
		...
		user_cert = sfra.updateCertificate ( user_id, user_name, "KR", address, company_name, company_unit, email ); 

		out.println("<html>");
		out.println("<head>");
		out.println("<title>인증서 변경 결과</title>");
		...
		if ( user_cert != null ) {
			...
			out.println("  <p><font size='6'><font color='#6699CC'><b>사용자 인증서 변경 결과</b></font></font></p>");
			out.println("  <p>&nbsp;</p>");
			out.println("    <p>사용자 : " + user_name + " 님 </p>");
			out.println("    <p>계좌번호 : " + account + "</p>");
			out.println("    <p>사용자 ID : " + user_id + "</p>");
			out.println("    <p>" + request.getParameter("user_name") + "'님의 사용자 인증서가 소프트포럼 인증기관으로 부터 변경 되었습니다. </p><br>");
			...
			out.println("<p>		사용자 인증서 변경 실패했습니다.</p>");
