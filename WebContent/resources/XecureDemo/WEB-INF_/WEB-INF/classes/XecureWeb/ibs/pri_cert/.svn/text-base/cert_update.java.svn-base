...
public class cert_update extends XecureHttpServlet 
{ 
		...
		out.println("<html>");
		out.println("<head>");
		out.println("<title>인증서 변경</title>");
		...
		out.println("<p><font size='6'><font color='#6699CC'><b>소프트포럼 인증기관 : 인증서 변경 신청</b></font></font></p>");
		out.println("<p><font color='#000000' size='5'>사용자 확인을 위해서 아래의 정보를 입력해 주십시오.</font></p>");
		out.println("<form name='form1' method=post action='XecureWeb.ibs.pri_cert.cert_update_op' onSubmit='return XecureSubmit(this);'>");
		out.println("<table>");
