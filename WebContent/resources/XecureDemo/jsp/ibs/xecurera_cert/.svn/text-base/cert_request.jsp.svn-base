...
<%
	...
	ra_result = ra_client.registerNewUser ( operaterId, user_type, company_name, 
...
{
	nRet = RequestCertificate( 11, form.ref_code.value, form.auth_code.value );
	if ( nRet == 0 )
...
<tr><td>등록일</td><td><%=ra_client.getResponseFromRegUser("REGDATE")%></td></tr>
<tr><td>등록번호</td><td><%=ra_client.getResponseFromRegUser("REGSERIAL")%></td></tr>
<tr><td>사용자 ID</td><td><%=ra_client.getResponseFromRegUser("USERID")%></td></tr>
<tr><td>주민등록번호</td><td><%=ra_client.getResponseFromRegUser("SSN")%></td></tr>
<tr><td>사용자 이름</td><td><%=ra_client.getResponseFromRegUser("USERNAME")%></td></tr>
<tr><td>참조코드</td><td><%=ra_client.getResponseFromRegUser("REFCODE")%></td></tr>
<tr><td>인가코드</td><td><%=ra_client.getResponseFromRegUser("AUTHCODE")%></td></tr>
<tr><td>인증서 용도</td><td><%=ra_client.getResponseFromRegUser("CERTTYPE")%></td></tr>
<tr><td>인증서 등급</td><td><%=ra_client.getResponseFromRegUser("CERTCLASS")%></td></tr>
</table>
<br>
...
<input type=hidden name='ref_code' value='<%=ra_client.getResponseFromRegUser("REFCODE")%>'>
<input type=hidden name='auth_code' value='<%=ra_client.getResponseFromRegUser("AUTHCODE")%>'>
<input type=hidden name='class_id' value='<%=ra_client.getResponseFromRegUser("CERTCLASS")%>'>
...
%>
<p>		사용자 등록에 실패했습니다.</p>
에러 코드 : <%=ra_client.getLastError()%><br>
에러 메시지 : <%=ra_client.getLastErrorMsg()%><br>
xecConfig.getXecureRaHost() : <%= xecConfig.getXecureRaHost()%> <br>
xecConfig.getXecureRaPort() : <%= xecConfig.getXecureRaPort()%> <br>
