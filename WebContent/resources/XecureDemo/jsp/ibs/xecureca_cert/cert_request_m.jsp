...
<%
	...
	int      key_recover_type = Integer.parseInt(request.getParameter("key_recover_type"));
	...
	ca_result = ca_client.registerNewUserM ( operaterId, user_type, company_name, 
																   user_name, user_id, ssn, policy_type, 
																	email,cell_phone, fax, zipcode, address, phone,
																	czipcode, caddress, cphone,
																	cert_type, cert_class, corp_code, org_code, orgid_code );
...
function cert_request( form )
{
	nRet = RequestCertificate( "15", form.ref_code.value, form.auth_code.value );
	if ( nRet == 0 )
		confirm("XecureCA 인증기관으로 부터 키복구용 인증서를 발급 받았습니다.");
}
</script>
<%
}
