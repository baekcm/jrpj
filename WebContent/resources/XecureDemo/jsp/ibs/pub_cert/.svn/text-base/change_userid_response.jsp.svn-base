...
<%
	int	ra_result;
	String old_userid = request.getParameter("old_user_id");
	String new_userid = request.getParameter("new_user_id");

	PublicRA sfra = new PublicRA ( ((XecureHttpServletRequest)request).getXecureSession().getXecureConfig() );

	ra_result = sfra.changeUserID (old_userid, new_userid );

%>

<html>
<head>
<title>사용자 ID 변경</title>
...
<%
	if ( ra_result == 0 ) {
%> 
<%=sfra.getResponseFromChangeUserID("MESSAGE")%><br>
<%=sfra.getResponseFromChangeUserID("CODE")%><br>

<%	}
	else {
%>
<p>	사용자 아이디 변경에 실패했습니다.</p>
