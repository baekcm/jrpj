...
<%
	...
	if(company_name == null || company_name.length() == 0) company_name = "softforum";
	if(company_unit == null || company_unit.length() == 0) company_unit = "xecure";
	PrivateRA sfra = new PrivateRA ( ((XecureHttpServletRequest)request).getXecureSession().getXecureConfig() );
	Certificate user_cert;
	user_cert = sfra.requestCertificate ( cert_req, user_id, user_name, "KR", address, company_name, company_unit, email ); 
%>


<html>
<head>
<title>인증서 발급 결과</title>
...
%> 
    ...
    <p>계좌번호 : <%=account%></p>
    <p>사용자 ID : <%=user_id%></p>
    <p>cert_req : <%=cert_req%></p>
    <p><%=request.getParameter("user_name")%>'님의 사용자 인증서가 소프트포럼 인증기관으로 부터 발급 되었습니다. </p><br>

<script language='javascript'>
<%=user_cert.certInstallScript(user_cert.INSTALL_ENCRYPTED_USER_CERT)%> 
</script>
