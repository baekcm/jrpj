...
<%
	FileSigner signer = new FileSigner(this.getXecureConfig());

	RandomAccessFile f = new RandomAccessFile(request.getParameter("aCert"), "r");
	byte[] bpem = new byte[(int)f.length()];
	f.read(bpem);
	f.close();
	
	RandomAccessFile f1 = new RandomAccessFile(request.getParameter("aKey"), "r");
	byte[] bkey = new byte[(int)f1.length()];
	f1.read(bkey);
	f1.close();

	String passwd = request.getParameter("aPasswd");
	byte bpasswd[] = passwd.getBytes();	

	int err = signer.signFileAddWithKey(bpem, bkey, bpasswd,
										request.getParameter("fileIn"),
										request.getParameter("fileOut"));
%>

<title>전자서명 결과(FILE)</title>
            ...
            <table width="85%" border="0" cellspacing="0" cellpadding="0">
              <tr> 
                ...
                <td height="120" valign="middle"> 
                  <p align="center"><font size="5" face="Times New Roman, Times, serif">전자서명 결과입니다.</font></p>
                </td>
              </tr>
              <tr> 
                <td>&nbsp;</td>
...
<!---BEGIN_ENC--->
<form name=envelopeForm method=post action='fileVerifyMulti_result.jsp' onSubmit='return XecureSubmit(this);'>
...
<pre>
<%
	int	result = signer.getLastError();
	if ( result != 0 ) {
		out.println("오류가 있습니다.<br>");
		out.println("오류 번호 : " + signer.getLastError() + "<br>");
		out.println("오류 내용 : " + signer.getLastErrorMsg() + "<br>");
	}
	else {
...
%>
<tr>
<td width='17%'><strong>전자서명 내용</strong></td>
<td width='83%'><strong>
<pre>
<%
	out.println ( "원본 화일 :"+request.getParameter("fileIn") );
...
<tr>
<td><strong>서명 검증할 화일 경로</strong>
</td>
<td>
<input type=text name='filePath' size=80 value='<%=request.getParameter("fileOut")%>'></input>
</td>
</tr>
<tr>
