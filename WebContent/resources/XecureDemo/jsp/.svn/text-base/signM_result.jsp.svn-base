...
<%
	...
	if ( nVerifierResult != 0 ) {
		...
		out.println("서명 확인 성공 (인증서 검증 제외)<br>");
	}
%>
</strong></td>
</tr>
...
<pre>
<%
	// 인증서 정보를 추출. 자세한 사항은 Certificate 클레스 참고
...
<tr>
<td width='17%'><strong>공개키 알고리즘 ID</strong></td>
<td width='83%'><strong>
<pre>
<%
...
<td width='83%'><strong>
<pre>
<%=(verifier.getSignTimeMsg().equals(""))?"서명 시간 정보가 없습니다.":verifier.getSignTimeMsg()%>
</pre>
</strong></td>
</tr>
<tr>
<td width='17%'><strong>인증서 검증</strong></td>
<td width='83%'><strong>
<pre>
<%
	// 본 예제와 인증서 정보를 참고하여 적절히 분류
	// 이 예제는 GPKI인증서에 대한 정확한 분류가 아님
	if(subjectDN_o.equals("Government of Korea")){
		out.println("'o=Government of Korea'인 인증서 입니다. 인증서 검증을 별도로 수행하세요.");
	}else{
		out.println("'o=Government of Korea'인 인증서가 아닙니다. 인증서 검증을 수행합니다.");
		int ret = verifier.verifyCertificate();
		if(ret==0)
			out.println("인증서 검증 성공");
		else{
			out.println("에러코드 : "+ ret);
			out.println("에러메세지 : "+ verifier.getLastErrorMsg());
		}
	}
%>
</pre>
