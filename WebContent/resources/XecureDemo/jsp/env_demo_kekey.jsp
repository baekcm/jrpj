...
<%
		...
		envelope = new EnvelopeData(this.getXecureConfig());
		enveloped = envelope.keKeyEnvelopeData_CMS(request.getParameter("passwd").getBytes(),request.getParameter("plain"));
%>

<title>전자통투 암호화 결과(패스워드)</title>
            ...
            <table width="85%" border="0" cellspacing="0" cellpadding="0">
              <tr> 
                ...
                <td height="120" valign="middle"> 
                  <p align="center"><font size="5" face="Times New Roman, Times, serif">전자봉투 암호화 결과입니다.(패스워드)</font></p>
                </td>
              </tr>
              <tr> 
                <td>&nbsp;</td>
...
<!---BEGIN_ENC--->
<form name=envelopeForm method=post action='env_demo_result_kekey.jsp' onSubmit='return XecureSubmit(this);'>
...
<%
	int i;
	for ( i = 0; i < enveloped.length(); i+=80 ) {
		if ( i + 80 < enveloped.length() ) 
			out.println ( enveloped.substring( i, i+80 ) );
		else
			out.println ( enveloped.substring( i, enveloped.length() ));
	}
%>
</pre>
</strong></td>
...
<tr>
<td><strong>비밀번호</strong>
</td>
<td>
<input type=text name='pwd' value='<%=request.getParameter("passwd")%>'>
</td>
</tr>
<tr>
<td colspan=2 align=center>
<input type=hidden name='enveloped' value='<%=enveloped%>'>
