...
<%
	...
	String fileOut = request.getParameter("fileOut");
	String certPath = "/user3/ucme/xecureweb_ver6/demo/file/signCert.der";
	String keyPath = "/user3/ucme/xecureweb_ver6/demo/file/signPri.key";
	String pwdFile = "/user3/ucme/xecureweb_ver6/demo/file/signPri.pwd";
	boolean ret=false;

	XecureFileDecrypt decrypt=null;

	try{
		decrypt = new XecureFileDecrypt(fileIn,
		                      certPath.getBytes(), keyPath.getBytes(),
							  XecureFileDecrypt.getPwdFromFile(pwdFile));

		ret = decrypt.writeFile(fileOut, true);
	}catch(Exception e){
...
/*	ByteArrayOutputStream byteOut = null;
	...
	try{
		...
		long envLen = new File(fileIn).length();
		out.println("Env Size :"+envLen+"<br>");
		byteOut = new ByteArrayOutputStream((int)envLen);
		ret = decrypt.writeStream(byteOut);
	}catch(Exception e){
		out.println(e.getMessage());
	}

	if(ret){
		out.println("Size :"+byteOut.size()+"<br>");
		out.println("Size :"+byteOut.toByteArray().length+"<br>");
...
<pre>
<%
	out.println(decrypt.getLastErrorMsg());
	}
%>
</pre>
</strong></td>
