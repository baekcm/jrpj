	...
	function fnLoad(){
		...
		if("${memberCnt}" == "0" ){
			if("1" == "${iRtn}") {
				...
				if("${adult}" == "child" && ("${sAgeCode}"=="0" || "${sAgeCode}"=="1" || "${sAgeCode}"=="2")){
					...
					if("${adult}" == "child" ){
						...
						$("#vnoform",opener.document).attr("action","/front/certy/online");
							...
							if("${sAgeCode}"=="0" || "${sAgeCode}"=="1" || "${sAgeCode}"=="2"){
						...
						if("${type}" == "off"){
							...
							$("#parm",opener.document).val("1");
						}else{
							$("#vnoform",opener.document).attr("action","/formMemberJoinInput");
						}
					}
					$("#vnoform",opener.document).submit();
				}
