...
/* 			if($("#searchKeySlct").val() == "all") {
			...
			$("#searchVal").val($.trim($("#searchValTxt").val()));
			$("#empSeq").val("");
			$("#empDept").val($.trim($("#empDeptSlct").val()));
			$("#pageFrm").attr("action","/front/listEmp").submit();
		});
	})
			...
			<div class="contents"> 
				...
				<div class="pageSection"> 
					...
					<div class="pageTopSearch">
						<div class="fl">
							<select name="empDeptSlct" id="empDeptSlct" title="부서 카테고리">
								<option value="">전체</option>
								<c:if test="${ listDept != null && fn:length(listDept) > 0 }">
									<c:forEach items="${ listDept }" var="list">
										<option value="${ list.empDept }" <c:if test="${ list.empDept eq empDept }">selected="selected"</c:if> >${list.empDept }</option>
									</c:forEach>
								</c:if>
							</select>
...
<script type="text/javascript">initMenu(7,4,2,0);</script>
</body>
</html>