	...
	<div id="contbody">
		...
		<div id="contentcore">
			...
			<div class="contents"> 
				...
				<!-- tabMaker  --> 
				<!-- real Contents -->
				<form action="/front/updateSugest" method="post" name="frm" id="frm" enctype="multipart/form-data">
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" summary="제목,작성자, 습득 및 분실일, 습득 및 분실장소, 보관 유무와 내용으로 이루어진 정보를 기입하기 위한 표" class="bd00view">
						...
						<tr>
							...
							<td class="bd01td"><span class="bd01td listx">
								<input readonly="readonly" name="userNm" id="userNm" type="text" value="${outBbs.userNm }" />
								</span></td>
						</tr>
						<%-- <tr>
							<th class="ta_l" scope="row">제안구분</th>
							<td class="bd01td">
								<label for="divsion"><input type="radio" name="suggestGbnCd"  <c:if test="${outBbs.suggestGbnCd eq '1' }">checked="checked" </c:if> id="divsion" value="1" />
								<strong>공단직원</strong></label> &nbsp; 
								<label for="divsion2"><input type="radio" name="suggestGbnCd"  <c:if test="${outBbs.suggestGbnCd eq '2' }">checked="checked" </c:if> id="divsion2" value="2" />
								<strong>주민제안</strong></label></td>
						</tr> --%>
						<tr>
									<th class="ta_l" scope="row"><label for="telno1">연락처</label></th>
									<td class="bd01td" >
									<select id="phone1Slct" name="phone1Slct" style="width: 80px;" title="연락처 앞자리">
										<option value="">선택</option>
										<c:forEach items="${phoneList }" var="phone">
											<option value="${phone.codeNm }"
												<c:if test="${outBbs.phone1 eq phone.codeNm }">selected="selected"</c:if>
											>${phone.codeNm }</option>
										</c:forEach>
									</select>
									<input value="${outBbs.phone1 }" type="hidden"  name="phone1" id="phone1" title="연락처 앞자리" />
										- 
										<input maxlength="4" size="5" name="phone2"id="phone2" value="${outBbs.phone2 }" title="연락처 중간자리" />
										- 
										<input maxlength="4" size="5" name="phone3" id="phone3" value="${outBbs.phone3 }" title="연락처 뒷자리" /></td>
								</tr>
								<tr>
									<th class="ta_l" scope="row"><label for="email">E-MAIL</label></th>
									<td class="bd01td" >
										<input maxlength="30" size="30" name="email1" id="email1" value="${outBbs.email1 }" />@
										<input maxlength="30" size="30" name="email2" id="email2" readonly="readonly" value="${outBbs.email2 }" />
									<select id="emailSlct" name="emailSlct" style="width: 120px;" title="이메일 도메인 선택">
										<option value="">선택</option>
										<c:forEach items="${mailList }" var="mail">
											<option value="${mail.codeNm }"
												<c:if test="${outBbs.email2 eq mail.codeNm }">selected="selected"</c:if>
												<c:if test="${fn:trim(outBbs.email2) != '' && fn:trim(outBbs.email2 ) eq mail.codeNm}">
								...
								<tr>
									...
									<td class="bd01td">
										<label for="divsion"><input type="radio" name="useYn" id="useYn1" <c:if test="${outBbs.useYn eq 'Y' }">checked="checked" </c:if> value="Y" />
										<strong>공개</strong></label> &nbsp; 
										<label for="divsion2"><input type="radio" name="useYn" id="useYn2" <c:if test="${outBbs.useYn eq 'N' }">checked="checked" </c:if> value="N" />
										<strong>비공개</strong></label></td>
								</tr>
						<tr>
							<th scope="row" class="ta_l"><label for="btitle">제안제목</label></th>
							<td class="bd01td"><input name="subject" id="subject" value="${outBbs.subject }"  size="73" maxlength="60" align="absmiddle" /></td>
						</tr>
						<tr>
							<th class="ta_l" scope="row">내용</th>
							<td class="bd01tdD"><ul class="guide">
									<li><strong>개요/목적</strong>
										<p><textarea name="purpose" id="purpose" cols="70" rows="10" title="개요/목적 입력">${outBbs.purpose }</textarea></p>
									</li>
									<li><strong>현황/문제점</strong>
										<p><textarea name="problem" id="problem" cols="70" rows="10" title="현황/문제점 입력">${outBbs.problem }</textarea></p>
									</li>
									<li><strong>개선방안</strong>
										<p><textarea name="way" id="way" cols="70" rows="10" title="개선방안 입력">${outBbs.way }</textarea></p>
									</li>
									<li><strong>기대효과</strong>
										<p><textarea name="effect" id="effect" cols="70" rows="10" title="기대효과 입력">${outBbs.effect }</textarea></p>
									</li>
								</ul></td>
						</tr>
						<tr>
							...
							<td class="bd01td">
								<input name="boardFileList" type="file" /><br/>
							</td>
						</tr>
						<tr>
							<td class="bd01td" colspan="2">
								<a href="#">${outBbs.orgnlFileNm}</a>
							</td>
						</tr>
					</table>
				</div>
