...
/* 			if($("#searchKeySlct").val() == "all") {
	...
	function popList(deptNm){
		var left = (screen.width - 450) / 2;
	    var top = (screen.height - 550) / 2;
	    $("#empDept").val(deptNm);
		window.open("about:blank","listEmp",'width=740,height=644,left=0,top=0,scrollbars=no');
		$("#pageFrm").attr("target","listEmp").attr("action","/front/listEmpPop").submit();
	}
</script>
</head>
...
<script type="text/javascript">initMenu(7,4,1,0);</script>
</body>
</html>