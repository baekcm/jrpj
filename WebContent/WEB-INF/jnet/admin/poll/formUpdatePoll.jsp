...
<script type="text/javascript">
	$(function(){ 
		$("input[name=questionNo]").each(function(){
			var problemSeq = $(this).attr("data");
			$(this).val($("tr.tr"+problemSeq).size());
		});
	...
	$("#problemBtn").click(function(){
		...
		problemTag += '		</select>';
		problemTag += '		<span class="problemSpan" style="display:none;">문항수 : <input type="text" name="questionNo" size="2" />';
	...
	$(document).on("click","input[type='button'].applyBtn",function(){
		...
		for(var i=0; i<appendCnt; i++) {
			...
			questionTag +='			<input type="text" name="qContentArr" />';
...
<div id="middle">
	<div id="contbody">
		...
		<div id="contentcore">
			...
			<div class="contents"> 
				<div class="boardWrap">
                	<form action="/admin/updatePoll?gseq=${gseq }&lseq=${lseq}" method="post" name="frm" id="frm">
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" summary="제목,작성자, 습득 및 분실일, 습득 및 분실장소, 보관 유무와 내용으로 이루어진 정보를 기입하기 위한 표" class="bd00view" id="ctsTbl">
							...
							<tr>
								...
								<td class="bd01td">
									<input type="text" size="60" name="subject" id="subject" style="height: 18px" value="${findPoll.subject}" />
								</td>
							</tr>
							<tr>
								<td colspan="2" scope="row" class="bd01tdC">
									<textarea name="contents" id="contents" cols="60" rows="30" title="내용입력" style="width:100%; height:300px; ">${findPoll.contents}</textarea>
									<script type="text/javascript">
										loadEditor("contents");
									</script>
								</td>
							...
							<tr>
								...
								<td class="bd01td">
									<select id="useYn" name="useYn">
										<option value="Y"
											<c:if test="${findPoll.useYn eq 'Y' }">selected="selected"</c:if>
										>저장</option>
										<option value="R"
											<c:if test="${findPoll.useYn eq 'R' }">selected="selected"</c:if>
										>임시저장</option>
									</select>
								</td>
							</tr> 
							<tr>
								...
								<td class="bd01td">
									<input type="text" size="10" name="stDtm" id="stDtm" style="height: 18px" readonly="readonly" value="${fn:substring(findPoll.stDtm,0,4)}-${fn:substring(findPoll.stDtm,4,6)}-${fn:substring(findPoll.stDtm,6,8)}" />
									~
									<input type="text" size="10" name="edDtm" id="edDtm" style="height: 18px" readonly="readonly" value="${fn:substring(findPoll.edDtm,0,4)}-${fn:substring(findPoll.edDtm,4,6)}-${fn:substring(findPoll.edDtm,6,8)}"  />
									<input type="button" id="problemBtn" value="질문추가" />
								</td>
							</tr>
							<c:if test="${listPollQuestion != null && fn:length(listPollQuestion) > 0 }">
								<c:set var="cnt" value="0" />
								<c:set var="beforeProblemType" value="" />
								<c:forEach items="${listPollQuestion }" var="listPollQuestion" varStatus="status">
									<c:if test="${listPollQuestion.prntProblemSeq eq 0 && status.index > 0 && beforeProblemType != '1' }">
										<c:set var="tagend" value="</table></td></tr>"></c:set>
										<!-- 내용이 들어가야함 -->
										${tagend }
									</c:if>
									...
									<c:choose>
										<c:when test="${listPollQuestion.prntProblemSeq eq 0 }">
											<tr class="problemNo">  
												 <th class="ta_l" scope="row">질문</th>
												 <td class="bd01td">
												 	<input type="text" size="40" name="problemContentArr" style="height: 18px" value="${listPollQuestion.problemContent }" />
												 	<input type="hidden" name="questionCntArr" />
														<select name="problemTypeArr">
															<option value="">선택</option>
															<option value="0"
																<c:if test="${listPollQuestion.problemType eq 0 }">selected="selected"</c:if>
															>객관식</option>
															<option
																<c:if test="${listPollQuestion.problemType eq 1 }">selected="selected"</c:if>
															 value="1">주관식</option>
														</select>
														<span class="problemSpan" <c:if test="${listPollQuestion.problemType eq 1 }">style="display:none;"</c:if>>
															문항수 : <input type="text" data="${listPollQuestion.problemSeq }" name="questionNo" size="2" />
														<input type="button" class="applyBtn" value="적용" /></span>
														<input type="button" class="removeBtn" value="삭제" />
												</td>
											</tr>
											<c:set var="cnt" value="${cnt+1 }" />
											<c:set var="beforeProblemType" value="${listPollQuestion.problemType }"/>
										</c:when>
										<c:otherwise>
											<c:if test="${cnt > 0 }">
											<c:set var="tag1" value='<tr><td colspan="2"><table class="questionTbl bd00view" width="100%" border="0" cellspacing="0" cellpadding="0"><colgroup><col width="150px" /><col /></colgroup>'></c:set>
											<!-- 내용이 들어가야함 -->
											${tag1 }
											</c:if>
											<c:set var="cnt" value="0" />
											<tr class="tr${listPollQuestion.prntProblemSeq } ctsTr">
												<th  class="ta_l" scope="row">
													내용
												</th>
												<td class="bd01td">
													<input type="text" name="qContentArr" value="${listPollQuestion.problemContent }" />
													<select name="qNoTypeArr">
														<option value="0"
															<c:if test="${listPollQuestion.qNoType eq 0 }">selected="selected"</c:if>
														>선택</option>
														<option value="1"
															<c:if test="${listPollQuestion.qNoType eq 1 }">selected="selected"</c:if>
														>기타</option>
													</select>
													<input type="button" class="rmBtn" value="삭제" />
												</td>
											</tr>	
											<c:set var="tmp" value="${listPollQuestion.prntProblemSeq }" />
				...
				<div class="btn_farm2 mt20 mb20"> 
					...
					<span class="txtBtn_lightGray"><a href="#" id="saveBtn" >수정</a></span> &nbsp; 
					<!-- Text Button --> 
				</div>
			</div>
		</div>
